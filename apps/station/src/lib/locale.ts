...
export const locale = {
  ...
  license: {
    ...
    /** Kept on the manager PC: nothing on this tablet can lose it. */
    heldBody:
      'حُفظت على جهاز المدير، وتُضاف إلى سجل الزبون تلقائياً عند تفعيل البرنامج — حتى لو أُعيد تشغيل هذه المحطة أو مُسحت بياناتها. لا خصم على هذه الفاتورة، لا الآن ولا بعد التفعيل — تابع خدمة الزبون كالمعتاد.',
    /** The manager PC could not keep it, so this station does until it can hand it over. */
    heldHereBody:
      'حُفظت على هذه المحطة لأن جهاز المدير تعذّر عليه حفظها، وتُرسل إليه تلقائياً إلى أن يُفعَّل البرنامج — لا تمسح بيانات المتصفح على هذه المحطة. لا خصم على هذه الفاتورة، لا الآن ولا بعد التفعيل.',
    /** The discount this invoice would have earned — shown, because it is not given later. */
    forgone: (amount: string) => `الخصم الذي كانت هذه الفاتورة تستحقه: ${amount} — لن يُطبَّق.`,
    /**
     * The one sentence the cashier can say to the customer at the till. True whatever
     * happens next: no discount on this basket (a discount the register did not apply
     * cannot be issued later), and the invoice is kept against his card.
     */
    sayLabel: 'قل للزبون:',
    say: 'عذراً، نظام الخصومات متوقف اليوم، فلا خصم على هذه الفاتورة — لكنها محفوظة على بطاقتك.',
    notHeldTitle: 'لم تُحتسب هذه الفاتورة ولم تُحفظ',
    notHeldBody: (cap: number) =>
      `هذه المحطة تحفظ ${cap.toLocaleString('en-US')} فاتورة كحدٍّ أقصى بانتظار التفعيل، وقد امتلأت. أبلغ المدير الآن ليفعّل البرنامج أو يتصل بالمزوّد لرمز طوارئ — وتابع خدمة الزبون كالمعتاد.`,
    sayNotHeld: 'عذراً، نظام الخصومات متوقف اليوم، فلا خصم على هذه الفاتورة، ولن تُسجَّل على بطاقتك.',
    strip:
      'البرنامج على جهاز المدير في وضع القراءة فقط: تُحفظ الفواتير على جهاز المدير وتُحتسب للزبائن عند التفعيل دون خصم، وتسجيل الزبائن الجدد متوقف مؤقتاً.',
    stripHeld: (count: number) => `الفواتير المحفوظة بانتظار التفعيل: ${count}.`,
    registerRefused:
  ...
  errors: {
    ...
    unexpected: 'تعذّر إكمال العملية على هذه المحطة. أعد المحاولة، وإن تكرّر ذلك فأعد تحميل الصفحة وأبلغ المدير.',
    sessionExpired: 'انتهت الجلسة — سجّل الدخول مرة أخرى',
    /** The browser refused to keep a sale for sending later. */
    queueNotSaved:
      'لم تُحفظ العملية على هذه المحطة لإرسالها لاحقاً — المتصفح رفض تخزينها. لم تُحتسب المشتريات؛ أبلغ الإدارة وتابع البيع كالمعتاد.',
  },
